...
class InitPeople(pyflamegpu.HostFunction):
    def run(self, FLAMEGPU):
        ...
        if(people.count() > 0):
            ...
            # People have not been predefined, set them up for a run here
            ...
            # Set journeyIndex so we know which element of the destination array we should use
            # Decrement the journeyIndex as each waypoint is reached. 
               
            # Start people at lower entrance
            for i in range(10):
                t = people.newAgent()                
                t.setVariableFloat("x", FLAMEGPU.random.uniformInt(230, 270)/10.0)
                t.setVariableFloat("y", 0.0)
                t.setVariableFloat("destx", 0, 0.1)     # Format used here to add a single item to an already existing array: "array-name", index, value
                t.setVariableFloat("desty", 0, FLAMEGPU.random.uniformInt(130, 170)/10.0)
                t.setVariableInt("dwell", 0, 0)
                t.setVariableInt("journeyIndex", 0)
                
                if(i == 3):
                    #Give this person a waypoint on their journey prior to their eventual destination
                    #Waypoint(s) are index 1+. The highest indexed waypoint will be visited first. 
                    t.setVariableFloat("destx", 1, 28.0)
                    t.setVariableFloat("desty", 1, 19.0)
                    t.setVariableInt("dwell", 1, 150) 
            ...
            #Start people at upper entrance
            for j in range(10):
                t = people.newAgent()
                
                t.setVariableFloat("x", 0.0)
                t.setVariableFloat("y", FLAMEGPU.random.uniformInt(130, 170)/10.0)
                t.setVariableFloat("destx", 0, FLAMEGPU.random.uniformInt(230, 270)/10.0)
                t.setVariableFloat("desty", 0, 0.0)
                t.setVariableInt("dwell", 0, 0)
                t.setVariableInt("journeyIndex", 0)
                t.setVariableFloat("velx", 1.0)
...
env.newPropertyFloat("timestep", 1)
...
env.newPropertyFloat("accuracy_ped", 0.25)
...
if(simulation.getEnvironmentPropertyInt("logging") >= 1):
    ...
    filename = "pedestrians_" + datetime.now().strftime("%Y%m%d-%H%M%S") + ".xml"
    simulation.exportLog(
	filename, # The file to output (must end '.json' or '.xml')
	simulation.getEnvironmentPropertyInt("logging")>=1, # Whether the step log should be included in the log file
	True, # Include exit log
	True, # Include step time in the log file (treated as false if step log not included)
	True, # Include simulation time in the log file (treated as false if exit log not included)
	False # Whether the log file should be minified or not
    )
    if(simulation.getEnvironmentPropertyInt("logging") >= 2):     
        # Log agent data
        filename = "agents_" + datetime.now().strftime("%Y%m%d-%H%M%S") + ".xml"
