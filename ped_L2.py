...
class InitPeople(pyflamegpu.HostFunction):
    def run(self, FLAMEGPU):
        ...
        if(people.count() > 0):
            ...
            light_zones = [
                   # xmin, xmax, ymin, ymax, light_score
                   (0.0,  5.0, 5.5, 7.0, 0.5),
                   (5.0, 10.0, 5.5, 7.0, 0.75),
                   (10.0, 15.0, 5.5, 7.0, 1.0),
                   (15.0, 20.0, 5.5, 7.0, 1.0),
                   (20.0, 25.0, 5.5, 7.0, 1.0),
                   (25.0, 30.0, 5.5, 7.0, 1.0),
                   (30.0, 35.0, 5.5, 7.0, 0.75),
                   (35.0, 40.0, 5.5, 7.0, 0.5),
            ]

            zone_counts = [0, 0, 0, 0, 0, 0, 0, 0]
            
            def choose_lighting_waiting_location():
                best_zone = 0
                best_score = -9999.0 #so when 

                
                w_light = 2.0 # Lighting is the main factor
                w_crowd = 0.4 # Crowd is a weaker attraction
                w_overcrowd = -2 # But too much crowd cause avoidence
                preferred_group_size = 3.0 # 3 people is the max for attraction

                zone_scores=[]


                for z in range(len(light_zones)):
                    xmin, xmax, ymin, ymax, light_score = light_zones[z]

                    count = zone_counts[z]

                    # Social attraction increases until preferred group size:3
                    crowd_score = min(count / preferred_group_size, 1.0) 
                    # 1 is devided by 3, the score increases 0.33 at every count, and stop at the third count(person)

                    # Once the zone becomes too crowded
                    overcrowd_score = max((count - preferred_group_size) / preferred_group_size, 0.0) 
                    # the score stays 0 until the third count, then increase 0.33 every count 

                    #The equation
                    score = (w_light * light_score) + (w_crowd * crowd_score)+(w_overcrowd * overcrowd_score)

                    zone_scores.append(score)

                    # Find highest score
                    best_score = max(zone_scores)
                    # Find ALL zones with the highest score
                    best_zones = []

                    for z in range(len(zone_scores)):
                       if abs(zone_scores[z] - best_score) < 0.0001:
                          best_zones.append(z)

                    # Randomly choose one from them
                    random_index = FLAMEGPU.random.uniformInt(0, len(best_zones)-1)
                    best_zone = best_zones[random_index]
                ...
                xmin, xmax, ymin, ymax, light_score = light_zones[best_zone]

                x = FLAMEGPU.random.uniformInt(int(xmin * 10), int(xmax * 10)) / 10.0
                y = FLAMEGPU.random.uniformInt(int(ymin * 10), int(ymax * 10)) / 10.0

                return x, y

            #Start people that come and wait
            for i in range(20):
                t = people.newAgent()

                start_x = FLAMEGPU.random.uniformInt(185, 215) / 10.0
                start_y = 0.8

                wait_x, wait_y = choose_lighting_waiting_location()
                ...
                t.setVariableInt("dwell", 1, 99999)
                t.setVariableInt("journeyIndex", 1) 
                t.setVariableFloat("velx", 0.0)
                t.setVariableFloat("vely", 1.0)
                t.setVariableFloat("steering", steering_ped * (FLAMEGPU.random.uniformInt(8, 12)/10))
            ...
            #Start people that get out of train walk to the stair
            for j in range(25):
                t = people.newAgent()

                start_x = FLAMEGPU.random.uniformInt(20, 380) / 10.0
                start_y = 7.5

                # destination (stair)
                dest_x = FLAMEGPU.random.uniformInt(185, 215) / 10.0
                dest_y = -1
                ...
                t.setVariableFloat("steering", steering_ped * (FLAMEGPU.random.uniformInt(8, 12)/10))
                t.setVariableInt("starttime", FLAMEGPU.random.uniformInt(0, 5000))
                t.setVariableFloat("maxSpeed", FLAMEGPU.random.uniformInt(20,23)/10.0)
                t.setVariableFloat("m", m_ped)
                t.setVariableFloat("n", n_ped)
                t.setVariableFloat("p", p_ped)
                t.setVariableFloat("cap", cap_ped)
...
if simulation.getEnvironmentPropertyInt("logging") >= 2:
    filename = os.path.join(output_dir, "agents_step_00000.xml")
...
for step in range(1, total_steps + 1):
    ...
    if simulation.getEnvironmentPropertyInt("logging") >= 2:
        if step % save_every == 0:
            filename = os.path.join(output_dir, "agents_step_{:05d}.xml".format(step))
            simulation.exportData(filename)
            print("Saved:", filename)

if simulation.getEnvironmentPropertyInt("logging") >= 1:
    filename = os.path.join(output_dir, "runlog_" + run_stamp + ".xml")
    simulation.exportLog(
        filename,
        simulation.getEnvironmentPropertyInt("logging") >= 1,
        True,
        True,
        True,
        False
...
if simulation.getEnvironmentPropertyInt("logging") >= 2:
    filename = os.path.join(output_dir, "agents_final.xml")
    simulation.exportData(filename)
    print("Saved final state:", filename)
