...
class InitPeople(pyflamegpu.HostFunction):
    def run(self, FLAMEGPU):
        ...
        if(people.count() > 0):
            ...
            #set distributed seats
            def choose_seat_waiting_location(i):
                seat = i % 4 #the seat will be choose in order, so people won't over choose one of them

                if seat == 0:
                    # seat1
                    x = FLAMEGPU.random.uniformInt(785, 965) / 100.0
                    y = FLAMEGPU.random.uniformInt(557, 620) / 100.0 # move away from the wall

                elif seat == 1:
                    # seat2
                    x = FLAMEGPU.random.uniformInt(1535, 1715) / 100.0
                    y = FLAMEGPU.random.uniformInt(557, 620) / 100.0

                elif seat == 2:
                    # seat3
                    x = FLAMEGPU.random.uniformInt(2285, 2465) / 100.0
                    y = FLAMEGPU.random.uniformInt(557, 620) / 100.0

                else:
                    # seat4
                    x = FLAMEGPU.random.uniformInt(3035, 3215) / 100.0
                    y = FLAMEGPU.random.uniformInt(557, 620) / 100.0
            ...
            for i in range(20):
                ...
                #half people starts from left door, half starts from right
                if i < 5:
                    start_x = 0.8
                    velx = 1.0
                else:
                ...
                # First 12 passengers use seats
                if i < seat_capacity:
                  wait_x, wait_y = choose_seat_waiting_location(i)
                
                # Remaining passengers wait near walls
                else:
                  wait_x, wait_y = choose_wall_waiting_location()
